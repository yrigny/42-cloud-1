...
3. `roles/mariadb`, `roles/wordpress`, and `roles/nginx` prepare their own service-specific build contexts.
4. `roles/webapp` renders the stack-level templates and starts the stack.
5. `group_vars/all.yml` holds the environment values and Vault-managed secrets used by the rendered files.
...
- `roles/mariadb`, `roles/wordpress`, and `roles/nginx` each own one service layer, which makes the stack more modular.
